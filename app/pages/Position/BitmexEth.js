...
class Bitmex extends React.PureComponent {
  ...
  render() {
    ...
    const config = {
      ...
      series: [
        {
          name: Trans({ en: 'Ethereum Price', zh: '以太坊价格', ko: '이더리움 가격' }),
          data: priceList,
          threshold: null,
          yAxis: 0,
        ...
        {
          name: Trans({ en: 'Open Interest (Conts)', zh: '未平仓合约数量(Conts)', ko: '미결제약정 수 (Conts)' }),
    ...
    return (
      <Wrapper>
        <Topi
          data={data}
          title={
            <I18n en="Bitmex Ethereum Perpetual Swap Contact Open Interest" zh="Bitmex以太坊永续掉期合约未平仓量" ko="Bitmex 이더리움 영구스왑컨트랙트 미결제약정" />
          }
        />
        <ToolWrapper>
          <Checkbox
          ...
          >
            <I18n en="Open Interest (Conts)" zh="未平仓合约数量(Conts)" ko="미결제약정 수 (Conts)" />
          </Checkbox>
        </ToolWrapper>
        ...
        <SlideUp title={<I18n id="介绍" />}>
          <DescTitle>
            <I18n en="Bitmex Ethereum Perpetual Swap Contact Open Interest" zh="Bitmex以太坊永续掉期合约未平仓量" ko="Bitmex 비트코인 영구스왑컨트랙트 미결제약정" />
          </DescTitle>
          <Desc>
            <I18n
              en="The ETHUSD swap has become the most liquid ETH/USD trading instrument globally. It allows speculators to trade ETH/USD risk, without ever touching Ether or USD. Like all BitMEX contracts, the margin and settlement currency for ETHUSD is Bitcoin. Open interest is the total number of open futures contracts that exist at a given time. When the open interest is at a high level, it means that a large amount of funds are betting on the price of the underlying property. The higher the amount of funds precipitated in futures contracts, the higher the motivation for the manipulation on spot prices, and the leverage effect of futures contracts may result in a big move in price."
              zh="ETHUSD 掉期已成为全球最具流动性的 ETH / USD 交易工具。它允许投资者交易着 ETH / USD 的价格，而不需要持有以太币或美元。与所有 BitMEX 合约一样， ETHUSD 的保证金和结算货币是比特币。未平仓量是指在特定时间存在的未平仓期货合约的总数。当未平仓量处于高位的时，意味着大量的资金在期货合约上进行价格博弈。期货合约沉淀资金量越高，主力操纵现货价格的动机越高，加上期货合约的杠杆效应，可能引发价格上的巨变。"
              ko="ETHUSD 스왑은 세계에서 가장 유동성이 있는 ETH / USD 거래 도구가 되었습니다.투자자들이 이더리움 또는 달러를 보유하지 않고도 ETH / USD 가격을 거래 할 수 있습니다.모든 BitMEX 계약과 마찬가지로 ETHUSD의 마진 및 결제 통화는 비트코인입니다.미결제약정은 특정 시간대의 미결제 선물계약의 총 수입니다.미결제가 높은 수준에 있을 때, 대규모의 자금이 선물계약에서 가격 줄다리기를 하고 있음을 의미합니다. 선물계약에 깔린 자금량이 많을수록 큰손이 현물가격을 조종하려는 동기가 높아지고 선물계약의 레버리지 효응까지 추가되어 가격의 큰 변동을 유발할 수 있습니다. "
            />
          </Desc>
        </SlideUp>
      </Wrapper>

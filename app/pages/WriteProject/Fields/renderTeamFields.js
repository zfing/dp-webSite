...
function RenderTeamFields({ fields }) {
  if (fields.length === 0) {
    fields.push({})
  }
  return (
        ...
        <Flex key={key} style={{ marginTop: key !== 0 ? '30px' : 0 }}>
          ...
          <Flex column flex={1} paddingLeft="28px">
            <Flex flex={1}>
              <Field
                name={`${team}.name`}
                placeholder={Trans({ id: '名称' })}
                component={TextField}
              />
              <Spacing />
              <Field
                name={`${team}.positionName`}
                placeholder={Trans({ id: '职位' })}
                component={TextField}
              />
            </Flex>
            <Flex marginTop="20px">
              <Field
                name={`${team}.linkedinUrl`}
                placeholder="Linkedln link"
                component={TextField}
              />
              <Spacing />
              <Field
                name={`${team}.facebookUrl`}
                placeholder="Facebook link"
                component={TextField}
              />
              <Spacing />
              <Field
                name={`${team}.twitterUrl`}
